...
<%@page import="com.ischoolbar.programmer.entity.common.*" %>
...
<div class="shop_goods_bd clear">
		...
		<div class="shop_goods_show">
			<div class="shop_goods_show_left">
				...
				<div id="preview">
					<div id="spec-n1"><IMG height="350" src="${product.imageUrl }" jqimg="${product.imageUrl }" width="350">
						</div>
						<div id="spec-n5">
			...
			<div class="shop_goods_show_right">
				<ul>
					...
					<li>
						<label>原价：</label>
						<span><strong>${product.ori_price }</strong>元&nbsp&nbsp&nbsp&nbsp&nbsp运费：卖家承担运费</span>
					</li>
					<li>
						<label>作者：</label>
					...
					<li>
						<label>出版社：</label>
						<span>${product.publisher }&nbsp&nbsp&nbsp&nbsp&nbsp出版时间：${product.pub_time } </span>
					</li>
					<li>
						<label>累计售出：</label>
						<span>${product.sellNum }件&nbsp&nbsp&nbsp&nbsp&nbsp浏览量：${product.viewNum }次</span>
					</li>
					<li>
						<label>评价：</label>
						<span>${product.commentNum }条评论&nbsp&nbsp&nbsp&nbsp&nbsp读者评分：${product.rating }分</span>
					</li>
					<li>
						<label>ISBN码：</label>
						<span><a href="javascript:void(0);" id="pr_isbn">${product.isbn }</a></span>
					</li>
					<li>
						<label>开本：</label>
						<span>${product.scale }&nbsp&nbsp&nbsp&nbsp&nbsp页数：${product.page_no }页</span>
					</li>
					<li class="goods_num">
						<label>购买数量：</label>
		...
		<div class="shop_bd_home_block_center" style="width:780px;">
                    <ul class="tabs-nav" style="width:780px;">
                        <li><a href="javascript:void(0);">看过这些也在看</a></li>
                    </ul>
                    <div class="tabs-panel" style="width:780px;height:200px">
                       <ul>
                            <c:forEach items="${perRecProductList }" var="product">
                ...
                <div class="shop_bd_home_block_center" style="width:780px;">
                    <ul class="tabs-nav" style="width:780px;">
                        <li><a href="javascript:void(0);">相似图书推荐</a></li>
                    </ul>
                    <div class="tabs-panel" style="width:780px;height:200px">
                        <ul>
                            <c:forEach items="${bookrecProductList }" var="product">
                            <li style="width:145px;height:200px;">
                                <dl>
                                    <dt><a href="../product/detail?id=${product.id }"><img style="width:158px;" src="${product.imageUrl}" /></a></dt>
                                    <dd><a href="../product/detail?id=${product.id }">${product.name }</a></dd>
...
<script>
	$(document).ready(function(){
		...
		$("#pr_isbn").click(function(){
			window.alert('11');
			/* window.alert($("#pr_isbn").value);
			window.alert($("lllaa").toString()); */
			var i_sbn =new isbn('9787550282766');
			/* var i_sbn =new isbn($("#product_isbn").text); */
			window.alert('12');
		});
	});
</script>
