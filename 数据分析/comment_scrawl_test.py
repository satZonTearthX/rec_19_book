...
class Mysql_service(object):
    ...
    def __init__(self):
        ...
        try:
            ...
            self.db.rollback()
        print("connection on")
    def information(self, id,productId, userId, type, content, createTime):  # self...
        sql = '''insert into product (id,productId, userId, `type`, content, createTime)\
               values(%d,%d,%d,%d,"%s") ''' % (id,productCategoryId, name, imageUrl, price, stock, sellNum, viewNum, commentNum, content, ISBN, author, publisher, pub_time, scale, page_no, rating, ori_price, barcode,productCategory)
        try:
            self.cursor.execute(sql)
            # 提交到数据库执行
            self.db.commit()
